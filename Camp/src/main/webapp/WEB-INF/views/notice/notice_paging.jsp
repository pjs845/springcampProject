    ...
    <script type="text/javascript"> 	
    	...
    	function tableInsert(){
    		...
    		{% for notice in notices %}
				var rdate = "{{notice.rdate|date:'Y년 m월 d일'}}";
				var jsonObject = {"id":"{{notice.id}}", "subject":"{{notice.subject}}", "writer":"{{notice.writer}}", "rdate": rdate};
    	...
    	function newPage(pageCurrent, pgcount){
    		...
    		if(pageCurrent == -1){ // 이전 페이지
    			...
    			for(var i=idx; i<tableList.length; i++){ //반복문을 수행하면서 tbody에 tr데이터 삽입 실시
                    ...
                    insertTr += "<tr>"; 
    				insertTr += "<td>"+id+"</td>"; 
    				insertTr += "<th id='th-subject' style='text-align:center;'>";
    				insertTr += "<a href='/notice/content/"+id+"'>"+subject+"</a>";
    				insertTr += "<p>테스트</p>";
