    ...
    <script type="text/javascript"> 	
    	...
    	function BoardList() { 
	    	...
	    	$.ajax({
				...
				type: "GET",  
				async: false,
				data:{catgo: "${catgo}", keyword: "${keyword}"},
    	...
    	function tableInsert(){
    		...
    		for (const board of boards){
				var jsonObject = {"seq":board.seq, "subject":board.title, "writer":board.name, "rdate": board.rdate, "hit":board.viewnum, "comments":board.comment_num};
				tableList.push(jsonObject);
				console.log("{{board.comments.count}}");
    		}
    	...
    	function newPage(pageCurrent, pgcount){
    		...
    		if(pageCurrent == -1){ // 이전 페이지
    			...
    			for(var i=idx; i<tableList.length; i++){ //반복문을 수행하면서 tbody에 tr데이터 삽입 실시
    				...
    				var id = jsonObject.seq;
    				var subject = jsonObject.subject;
    				var writer = jsonObject.writer;
					var rdate = jsonObject.rdate.substring(0, jsonObject.rdate.indexOf(" "));
                    var hit = jsonObject.hit;
    				...
    				var insertTr = ""; // 변수 선언
                    insertTr += "<tr>"; 
    				insertTr += "<td align='center'>"+id+"</td>"; 
					if (comments>0){
						insertTr += "<td><a href=\"/board/content.do?seq="+id+"\">"+subject+"</a>["+comments+"]</td>";
					}else{
						insertTr += "<td><a href=\"/board/content.do?seq="+id+"\">"+subject+"</a></td>";
					}		
    				insertTr += "<td>"+writer+"</td>";
    				insertTr += "<td>"+rdate+"</td>";
					insertTr += "<td>"+hit+"</td>";
    				insertTr += "</th>";                        
